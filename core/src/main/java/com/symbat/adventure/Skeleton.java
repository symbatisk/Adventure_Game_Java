...
public class Skeleton extends Enemy {
    ...
    public Skeleton(World world, float platformX, float halfWidth, float y) {
        super(world, platformX, halfWidth, y, 5);
    }

    private void loadTextures() {
        walkFrames = new ArrayList<>();
        for (int i = 0; i <= 8; i++) {
            walkFrames.add(new Texture("data/Skeleton/Walk/walk_" + String.format("%03d", i) + ".png"));
        }

        attackFrames = new ArrayList<>();
        for (int i = 0; i <= 6; i++) {
            attackFrames.add(new Texture("data/Skeleton/Slash/slash_" + String.format("%03d", i) + ".png"));
        }

        dieFrames = new ArrayList<>();
        for (int i = 0; i <= 5; i++) {
            dieFrames.add(new Texture("data/Skeleton/Dying/dying_" + String.format("%03d", i) + ".png"));
        }
    }
